...
This repository contains the source code for Interfold's entire frontend, including our mobile app and our website.
...
If you'd simply like instructions to run a development build of Interfold, scroll down to the "Development" page for
...
The Interfold app is built as a [Kotlin Multiplatform](https://kotlinlang.org/docs/multiplatform.html) project with the
[Compose Multiplatform](https://www.jetbrains.com/compose-multiplatform/) UI framework. As such, Interfold is largely
...
Instead of spending development time on techniques such as optimistic UI, Interfold uses
...
The root directory contains a Gradle project designed to centralize build dependencies across all of Interfold's submodules.
...
The vast majority of Interfold's code is written in common Kotlin contained in this submodule (`src/commonMain`).
This includes:
...
- Registering an iOS widget with the operating system (built with WidgetKit and SwiftUI)
  - Minimal Swift implementations of Interfold's relevant data models in Swift, for use with the widget's isolated networking layer
...
Running a development build of Interfold for iOS is much more complex than its other supported platforms. Generally, you can follow the following steps:
...
This directory contains the wrapper for the experimental WebAssembly ("Wasm") build of Interfold. This allows Interfold to be run in any web browser that supports modern Wasm
...
environment, such as the Interfold website. Instructions for this are [detailed below](#website).

Interfold for Wasm should run on any web page with a `<canvas id="ComposeTarget">` and the `interfold-app.js` script generated by Gradle imported. An example of this can be
...
In addition to the Interfold app, this repository also contains the source code for the Interfold website.
...
The Nuxt application embeds the Interfold web app in the form of a WebAssembly binary, which **must** be built through the
...
```bash
docker build -f Dockerfile.wasm -t interfold-wasm:latest .
```
