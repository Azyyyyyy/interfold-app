...
import app.interfold.glance.FrontWidgetWorker
...
class MainActivity : AppCompatActivity() {
  ...
  private val platformUtilities = object : PlatformUtilities {
    ...
    override fun clearServerConfigCache() {
      try {
        ...
        Log.e("INTERFOLD", "Failed to clear persisted public key: $e")
      }
    ...
    override suspend fun reinitPushNotifications(settings: Settings) {
      withContext(Dispatchers.Main) {
        try {
          ...
          Log.w("INTERFOLD", "FirebaseApp.delete failed (ignoring): $e")
        }
        ...
        if (!initialized) {
          Log.w("INTERFOLD", "Reinit: FirebaseApp not re-initialised; skipping token fetch")
          return@withContext
        }
        ...
        try {
          ...
          Log.e("INTERFOLD", "Reinit token fetch failed: $e")
        }
      }
    }
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    if (isLoggedIn) {
      ...
      GlobalScope.launch {
        try {
          ...
          Log.e("INTERFOLD", "Failed to preload public key: $e")
        }
      }
    }
    ...
    if (isLoggedIn) {
      GlobalScope.launch {
        ...
        if (!initialized) {
          Log.w("INTERFOLD", "Firebase not initialised (no cached config yet); skipping token fetch")
          return@launch
        }
        try {
          withTimeout(10_000L) {
          ...
          Log.e("INTERFOLD", "Failed to get Firebase token: $e")
        }
      }
    }
  ...
  private fun handleIntent(settings: Settings): Settings {
    ...
    platformLog("INTERFOLD", appLinkData?.toString() ?: "No data")

    val path = if (appLinkData?.scheme == "interfold") {
      // For interfold://auth/token, host is "auth" and path is "/token"
